...
export async function seedSections(
  ...
  const fixed1 = await prisma.pigeon.create({
    data: {
      ...
      color: "Alb",
      strain: "Janssen",
      titleRo: "Săgeata Albă — pui Janssen, preț fix",
      titleEn: "White Arrow — Janssen youngster, fixed price",
      descRo:
  ...
  const fixed2 = await prisma.pigeon.create({
    data: {
      ...
      color: "Vânăt pestriț",
      strain: "Van Loon",
      titleRo: "Perla Nordului — femelă de prăsilă, preț fix",
      titleEn: "Northern Pearl — breeding hen, fixed price",
